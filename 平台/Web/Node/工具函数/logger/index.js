...
const { consoleT, infoFileT, errorFileT, exceptionFileT, rejectFileT } = require('./type')
...
const logger = createLogger({
  ...
  exitOnError: true, // 异常时候是否退出:false 退出 true 不退出
  exceptionHandlers: null,
  rejectionHandlers: null,
})

if (process.env.NODE_ENV !== 'production') logger.add(consoleT)
if (process.env.NODE_ENV === 'production') {
  logger.exceptions.handle(exceptionFileT)
  logger.rejections.handle(rejectFileT) //处理未经批准的拒绝承诺
}
